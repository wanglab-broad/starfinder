...
W-170's `test/test_sample_export.py` combines the existing raster literals with
`artifact-contract-v1` molecular checkpoints (two source namespaces, one accepted
and one rejected read per source), explicit affine provenance, a two-gene integer
expression matrix and supplied cell assignments. The test-only association is
not a molecular recount or biological claim. Measured zero, unavailable
expression, unassigned/unavailable molecules and omitted traces remain separate.
A `(9,48,48)` single-channel `100*z+10*y+x` ramp with empty labels checks regional
chunk access (`1` chunk versus `8` for the full array). No RNG, real images or
historical TIFF regeneration is used. The external W-170 evidence pins saved
source/store hashes, environment, prepared levels and actual consumer outcomes;
serialization alone does not qualify Fiji interoperability.

The export link check also materializes the exact frozen three-molecule oracle:
`(A,7)`, `(B,7)`, `(A,8)` at `(0,0,0)`, `(0,0,2)`, `(0,1,1)` in the common
index frame. The third point is explicitly unassigned; the first two link to
