...
See the [documentation guide](docs/README.md) to build and preview the Sphinx site locally,
and [publishing guidance](docs/contributing.md#publish-and-verify-a-release) for release status checks.
Start with the [minimal Python quickstart](docs/getting-started.md) to generate
tiny synthetic images and process them to molecule-level CSVs on a local CPU.
The [Python package README](src/python/README.md) and [workflow examples](example/README.md)
remain available as entry points.
...
## Usage
For the Python backend, follow the [uv installation and quickstart](docs/getting-started.md).
For the existing Conda-based workflow demos, download the
[latest release](https://github.com/wanglab-broad/starfinder/releases) and create the workflow environment:
```bash
