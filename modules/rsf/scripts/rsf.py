...
# Helper for parsing protein stains into a string 
def parse_protein_stains(config_stains):
    out = "'protein_stains', ["
    for i in range(len(config_stains)):
        out += f'"{config_stains[i]}"'
        if i == len(config_stains)-1:
            out += "]"
        else:
            out += ", "
    return out
