...
ad_plot_stacked_bar <- function(interval_vec, #vector of cell min_dist info
                                ...
                                title = "",
                                dilate_vec, img_size, resolution_ = 0.18){
  sqpx_size_ <- resolution_^2
  ...
  for(i in names(dilate_vec)){
    tmp_df[tmp_df$interval == i,3] <- tmp_df[tmp_df$interval == i,3]/(dilate_vec[i]*sqpx_size_)
  }
  tmp_df[tmp_df$interval == "overall",3] <- tmp_df[tmp_df$interval == "overall",3]/(img_size*sqpx_size_)
  g <- ggplot(data = tmp_df,aes(x= interval, y=n, fill=label, order = label))+
    geom_bar(stat="identity") +
    labs(title = title) + ylab("Density (# per mm^2)")+
...
h5ad_to_seurat <- function(h5ad_path, layer_name, meta_col = NULL,
                           obs_index = "obs/_index", var_index = "var/_index"){
  file.h5 <- H5File$new(h5ad_path, mode="r")
  if(paste0(layer_name,"/indices") %in% list.datasets(file.h5)){
    # Sparse mat
    expr_mat <- sparseMatrix(i = file.h5[[paste0(layer_name,"/indices")]][] + 1,
                             p = file.h5[[paste0(layer_name,"/indptr")]][],
                             x = file.h5[[paste0(layer_name,"/data")]][],
                             dims = c(file.h5[[var_index]]$dims, file.h5[[obs_index]]$dims))
  }else{
    expr_mat <- file.h5[[layer_name]][,]
  }

  if(obs_index %in% list.datasets(file.h5)){
    dimnames(expr_mat) <- list(file.h5[[var_index]][],paste0("cell_",file.h5[[obs_index]][]))
  }else stop("No vaild cell index")

  meta_df <- data.frame(ind = paste0("cell_",file.h5[[obs_index]][]),
                        row.names = paste0("cell_",file.h5[[obs_index]][]))
  if(!is.null(meta_col)){
    obs_cols <- list.datasets(file.h5[["obs"]])
    for(i in meta_col){
      if(i %in% obs_cols){
        meta_df[[i]] <- file.h5[[paste0("obs/",i)]][]
      }else if(paste0(i,"/categories") %in% obs_cols & paste0(i,"/codes") %in% obs_cols){
        meta_df[[i]] <- file.h5[[paste0("obs/",i,"/categories")]][file.h5[[paste0("obs/",i,"/codes")]][]+1]
      }else{
        message(paste("Column:",i,"does not exist in the file!"))
      }
    }

  }
  seu_obj <- CreateSeuratObject(counts = expr_mat, meta.data = meta_df, min.cells = 0, min.features = 0)
  seu_obj
}
