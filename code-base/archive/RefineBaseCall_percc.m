function [pixelQual,baseCall] = RefineBaseCall_percc(registed_img,cc_stats)
%UNTITLED RefineBaseCall
%   test

Nround = size(registed_img, 1);
Ndots = size(cc_stats, 1);

pixelQual = zeros([size(registed_img{1}), Nround]);
baseCall = zeros([Ndots, 3*Nround]);
...
for r=1:Nround
    ...
    baseCall = [baseCall; curr_qual_centroid];
