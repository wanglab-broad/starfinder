function [barcode_matrix, quality_matrix] = GetBarcodesEachRound( input_round, Nchannel )
% GetBarcodesEachRound
% base call for each round

    % Calculate comp
    sq = input_round .^ 2;
    comp_denominator = sum(sq, 4) .^ 0.5; % dim4 - channel
    comp_denominator = repmat(comp_denominator, [1 1 1 Nchannel]);
    comp_matrix = input_round ./ comp_denominator; %4D
