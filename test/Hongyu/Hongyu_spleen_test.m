...
for i=fovs
    ...
    % current_fov = "Position001";
    current_fov = sprintf("Position%03d", i)
    ...
    input_path = fullfile('Z:/Data/Processed/2024-02-06-Hongyu-Hongyu_multicap_covid_spleen_replicates1/');
    output_path = fullfile('Z:/Data/Analyzed/2024-02-06-Hongyu-Hongyu_multicap_covid_spleen_replicates1/test/rsf_test_nohist_noLR_cluster/');
    ref_round = ["round4"];
    ...
    addpath('Z:/jiahao/Github/starfinder/code-base/new/') % pwd is the location of the starfinder folder
    ...
    % sdata = sdata.LocalRegistration('ref_layer', "round1");
    ...
    % Spot finding 
    % sdata = sdata.SpotFinding('ref_layer', "round4");
    % sdata = sdata.ReadsExtraction('voxel_size', [1 1 1]);
    % sdata = sdata.LoadCodebook;
    % sdata = sdata.ReadsFiltration('end_base', ["AC"]);
    % round4_ref_merge_max = max(round4_ref_merge, [], 3);
    % sdata = sdata.ViewSignal('bg_img', round4_ref_merge_max, 'save', true, 'output_path', fullfile(sdata.outputPath, "signal-round4"));
