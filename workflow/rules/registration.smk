...
def run_matlab_scripts(param_string, matlab_script_name):
    matlab_script_path = f"{config['starfinder_path']}/workflow/scripts/registration"
...
def get_runtime(wildcards, attempt):
    return attempt * config['rules']['global_registration']['resources']['runtime']
...
rule global_registration:
    ...
    params:
        uger_log=f"{OUTPUT_DIR}/log/global_registration/{{fovID}}.txt"
    threads: 4
    resources:
        mem_mb=config['rules']['global_registration']['resources']['mem_mb'],
        runtime=get_runtime
    benchmark:
        f"{OUTPUT_DIR}/log/benchmark/global_registration/{{fovID}}.txt"
    run:
        param_string = (f"'{input[0]}', "
                        f"'{wildcards.fovID}'"
                        )
        matlab_script_name = 'global_registration'
        run_matlab_scripts(param_string, matlab_script_name)

# Local registration
def get_runtime(wildcards, attempt):
    return attempt * config['rules']['local_registration']['resources']['runtime']
...
rule local_registration:
    ...
    output:
        temp(update(expand("{output_dir}/output/subtile/{{fovID}}/subtile_data_{{n_subtile}}.mat", output_dir=OUTPUT_DIR))),
    threads: 4
    resources:
        mem_mb=config['rules']['local_registration']['resources']['mem_mb'],
        runtime=get_runtime
    benchmark:
        f"{OUTPUT_DIR}/log/benchmark/local_registration/{{fovID}}_{{n_subtile}}.txt"
    run:
        param_string = (f"'{input[0]}', "
                        f"'{input[2]}'"
                        )
        matlab_script_name = 'local_registration'
        run_matlab_scripts(param_string, matlab_script_name)