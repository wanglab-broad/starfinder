...
def _prepare_geometry(config, shape, labels, stream):
    from ._formed import _array, _json
    ...
    if centers.ndim != 2 or centers.shape[1] != 3 or len(centers) > 1024:
        raise ValueError('centers_zyx must be K by 3 with K <= 1024')
    k = len(centers)
    ...
    effective = dict(type='GeometryConfig', translation_enabled=config.translation_enabled,
                     local_enabled=config.local_enabled, rounds=dict(zip(labels, maps)))
