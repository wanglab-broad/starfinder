...
def ext(value):
    return value.provenance['extensions']['starfinder.synthetic']
...
def test_combined_order_and_shared_background(size):
    ...
    assert ext(stationary)['background_components'] == ext(combined)['background_components']
    assert ext(combined)['background_components']
    assert ext(stationary)['observation']['pre_noise_sha256'] != ext(combined)['observation']['pre_noise_sha256']
...
def test_loss_dropout_and_noise_isolation():
    ...
    assert ext(original)['background_components'] == ext(changed)['background_components']
    assert ext(original)['streams'] == ext(changed)['streams']
