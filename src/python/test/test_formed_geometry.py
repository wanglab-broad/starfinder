"""Independent geometry expectations; no registration estimator is an oracle."""
...
def noise_streams(scene):
    return [s for s in scene.provenance['stream_scheme']['streams'] if s[4].startswith('noise.')]
...
def test_local_composition_shared_background_and_independent_inverse(depth):
    ...
    assert components(s)[0]['center_zyx'] == list(center)
...
def test_identity_disable_and_unrelated_latents_preserved():
    ...
    for other in (disabled, moved):
        ...
        assert components(base) == components(other)
        # Standardized draws are a pure function of these round/channel keys.
        assert noise_streams(base) == noise_streams(other) and len(noise_streams(base)) == 24
