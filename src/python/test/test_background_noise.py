...
def components(scene):
    return scene.provenance['effective_config']['background']['components']
...
def test_noise_pixel_oracle_and_isolation():
    ...
    for other in (noisy, changed):
        ...
        assert components(clean) == components(other)
        assert payload(clean)['transforms'] == payload(other)['transforms']
    streams = lambda s: payload(s)['stream_scheme']['streams']
    assert streams(noisy) == streams(changed)
    ...
    for a, b in zip(components(clean), components(other)):
...
def test_texture_laws_and_reference_evaluation():
    ...
    blobs = components(scene)
    assert len(blobs) == independent_draw('background.count').poisson(.01*3*7*9)
    for i, component in enumerate(blobs):
        identity = f'blob-{i}'
    ...
    np.testing.assert_array_equal(evaluate_background(components(scene),
...
@pytest.mark.parametrize('kwargs', [
    ...
    {'texture': TextureConfig(max_count=-1)},
...
def test_noise_cast_once_and_round_channel_streams():
    ...
    assert [s[4:] for s in streams] == [['noise.independent', None, r, c]
                                        for r in signed.round_labels for c in signed.channel_labels]
    assert streams == payload(cast)['stream_scheme']['streams']
